...
namespace OpenDataPortal.Core.DynamicQuery
{
    public static class IQueryableDynamicFilterExtensions
    {
        ...
        {
            IList<Filter> filters = GetAllWithoutPaginationFilters(filter);
            string?[] values = filters.Select(f => f.Value).ToArray();
            string condition = Transform(filter, filters);
            queryable = queryable.Where(condition, values);
            return queryable;

        }
        public static IList<Filter> GetAllWithoutPaginationFilters(Filter filter)
        {
